 ...
 *   - 選択すると即時フィードバック: 正解→補足、不正解→ヒント + 再挑戦可
...
export function StepQuiz({ quiz }: StepQuizProps) {
  ...
  // 試行回数（不正解 → 再挑戦でインクリメント）
  const [attempts, setAttempts] = useState(0)
  const emit = useRumCustomMetric()

  // ステップが変わったら（= 別の quiz になったら）状態をリセット
  // biome-ignore lint/correctness/useExhaustiveDependencies: stepId の変化をトリガにリセットする意図
  useEffect(() => {
    setSelectedId(null)
    setDismissed(false)
    setAttempts(0)
